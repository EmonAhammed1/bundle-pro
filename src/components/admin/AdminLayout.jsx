...
export const AdminLayout = ({ children }) => {
  ...
  useEffect(() => {
    // Check if running inside Shopify Admin iframe or URL contains shop/host param
    const isIframe = window.self !== window.top;
    const urlParams = new URLSearchParams(window.location.search);
    const hasShopParam = urlParams.has('shop') || urlParams.has('host');
    
    if (isIframe || hasShopParam) {
      setIsEmbeddedInShopify(true);
    }
  }, []);
  ...
  const sidebarNavItems = [
    ...
    { id: 'exporter', label: 'Settings', icon: Settings }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f7] text-slate-900 flex flex-col font-sans antialiased">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-semibold text-slate-500">Shopify Native App</span>
            </div>
          </div>
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto flex flex-col md:flex-row">
        ...
        {/* Left Sub-menu Sidebar: Shown on Standalone site, Hidden when Embedded in Shopify Admin iframe so Shopify's Native Left Panel takes over! */}
        {!isEmbeddedInShopify && (
          <aside className="w-full md:w-56 p-4 border-r border-slate-200 bg-white/70 space-y-4">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold transition ${
                      isActive
                        ? 'bg-slate-200/80 text-slate-900 font-bold shadow-inner'
